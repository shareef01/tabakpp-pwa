...
export const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message, confirmText = 'Confirm', confirmDisabled = false }) => (
  <ModalFrame isOpen={isOpen} onClose={confirmDisabled ? undefined : onClose}>
    {(titleId) => (
      <div className={cn(UI.MODAL_SHELL, 'max-w-[440px] p-8 md:p-10')}>
        <div className="flex justify-between items-start mb-8">
          ...
          <button type="button" onClick={onClose} disabled={confirmDisabled} aria-label="Close" className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-xl text-neutral-500 hover:text-white transition-colors disabled:opacity-40">
        ...
        <div className="flex flex-col gap-3">
          ...
          <Button variant="ghost" onClick={onClose} disabled={confirmDisabled} className="w-full h-14">Cancel</Button>
        </div>
      </div>
    )}
  </ModalFrame>

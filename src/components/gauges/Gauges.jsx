...
/**
 * Minimal Neon Glow (The Original Edge)
 */
const NeonGlow = ({ isOver }) => (
  <motion.div
    ...
    className={cn(
      "absolute -right-2 top-1/2 -translate-y-1/2 w-8 h-12 blur-xl rounded-full",
...
/**
 * RyoRollProgress (The Original Hand-Rolled)
 */
export const RyoRollProgress = React.memo(({ count, limit, size }) => {
  ...
  return (
    <div className={cn("relative flex items-center justify-start", isLarge ? "w-44 h-8" : "w-32 h-6")}>
      ...
      <div className="absolute right-[-8px] w-6 h-[40%] bg-neutral-800 rounded-sm z-20" />
    </div>
  );
});
...
/**
 * SmokingProgress (The Master Schachtel)
 */
